...
class ConjugateConstraintOptimizerPCPO_Discrete(Serializable):
    """
    ...
    def optimize(self, 
                 ...
                 #precomputed_eval_for_nonFixedhc=None,
        ...
        if self.precompute:
            S = precomputed_eval
            #S = precomputed_eval_for_nonFixedhc
        ...
        logger.record_tabular("Constraint_Violation", c)
        ...
        if optim_case > 0:
            flat_descent_step = (1. / (lam + eps) ) * ( v + nu * w ) # equation (13)
        else:
            # current default behavior for attempting infeasible recovery:
            # take a step on natural safety gradient
            flat_descent_step = np.sqrt(delta / (s + eps)) * w # w = H^{-1}b (equation 14)
        ...
        ### use cost as objective for App3, combined with the reward function (App2), this approach is for App4###
        # define the ratio of the safe policy
        alpha = 1e-2
        # This is to calcualte the lagrangain multiplier
        lam_ = max(0, np.sqrt((q - 2 * alpha * r + alpha * alpha * s ) / delta ))
        # compute the update direction
        flat_descent_step = (1. / (lam_ + eps) ) * ( v + alpha * w )
        # store the Lagrangain multiplier
        logger.record_tabular("OptimDiagnostic_lam_",lam_)
        # This is used to remove the line search procedure
        optim_case = -1 
        logger.record_tabular("OptimDiagnostic_lam_App4", np.sqrt(delta / (s + eps))) # dual variable for trust region
        ###############################################################################################
        '''
        ...
        ###############################################################################################
        ## Here, I modified the policy update direction
        ### Just use cost as objective for App3###
        flat_descent_step = - np.sqrt(delta / (s + eps)) * w
        optim_case = -1
        logger.record_tabular("OptimDiagnostic_lam_App3", np.sqrt(delta / (s + eps))) # dual variable for trust region
        ###############################################################################################
        '''
        ...
        if optim_case >= 0:
            ...
            lam_pg_KL = max(0, np.sqrt(delta / (q + eps)) * r + c + c_scale) / (s + eps)
            # KL projected gradient descent
            flat_descent_step_pg_KL = lam_pg_KL * w


            # combine these two directions
            flat_descent_step = flat_descent_step_tr + beta_soft * flat_descent_step_pg_KL
            #flat_descent_step = flat_descent_step_tr + 0.01 * w
            # store the Lagrangain multiplier

            logger.record_tabular("pcpo_reward",np.sqrt(delta / (q + eps)))
            logger.record_tabular("pcpo_reward q",q + eps)
            logger.record_tabular("pcpo_reward delta",delta)
            logger.record_tabular("pcpo_cost",lam_pg_KL)
            logger.record_tabular("pcpo_cost_scaledByBeta", beta_soft *lam_pg_KL)
            ...
            logger.record_tabular("pcpo_reward q",0)
            logger.record_tabular("pcpo_reward delta",0)
        ...
        ### Trust region safe policy optimization ###
        ### The following piece of the code is based on L2 projection ###

        # define the ratio of the safe policy
        alpha = 1e5 # 1e-3; 0.1; 1e1; 1000; 100000 1e5
        # This is to calcualte the lagrangain multiplier
        lam_ = max(0, np.sqrt((q - 2 * alpha * r + alpha * alpha * s ) / delta ))
        # compute the update direction
        flat_descent_step = (1. / (lam_ + eps) ) * ( v + alpha * w )
        ...
        flat_descent_step = np.sqrt(delta / (q + eps)) * v #sqrt(2*delta/g^tH^{-1}g)H^{-1}g
        param_tr = prev_param - flat_descent_step

        # when infeasible, take the step that purly decreases the objective value
        flat_descent_step = np.sqrt(delta / (s + eps)) * w #w = H^{-1}b (equation 14)
        param_sc  = prev_param - flat_descent_step

        if optim_case == 0:
            cur_param = 0.5 * param_tr + 0.5 * param_sc
        else:
            cur_param = 1.0 * param_tr + 0.0 * param_sc
        '''
        ...
        def wrap_up():
            if optim_case < 4:
                ...
                temp = cur_param - prev_param
                temp1 =  flat_b.dot(temp)
                next_linear_S = S + temp1# linear approximation (equation 11)
