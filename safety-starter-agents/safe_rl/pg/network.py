...
def placeholder_from_space(space):
    if isinstance(space, spaces.Dict):
        obs = space['image']
        shape = obs.shape[:-1] + (obs.shape[-1] + 2,)
        return placeholder(shape)
    elif isinstance(space, Box):
        return placeholder(space.shape)
    elif isinstance(space, Discrete):
        # return tf.range(space.n)
...
def mlp(x, hidden_sizes=(32,), activation=tf.tanh, output_activation=None):
    conv1 = tf.compat.v1.layers.conv2d(x, 5, 3, activation='relu')
    conv2 = tf.compat.v1.layers.conv2d(conv1, 6, 3, activation='relu')
    conv2_flattered = tf.compat.v1.layers.flatten(conv2)
    # x_flattered = tf.compat.v1.layers.flatten(x)
    x = tf.compat.v1.layers.dense(conv2_flattered, units=hidden_sizes[0], activation=activation)
...
def gaussian_kl(mu0, log_std0, mu1, log_std1):
    ...
    all_kls = tf.reduce_sum(input_tensor=pre_sum, axis=1)
    return tf.reduce_mean(input_tensor=all_kls)


def gaussian_entropy(log_std):
    """Returns average entropy over a batch of dists"""
    pre_sum = log_std + 0.5 * np.log(2 * np.pi * np.e)
    all_ents = tf.reduce_sum(input_tensor=pre_sum, axis=-1)
    return tf.reduce_mean(input_tensor=all_ents)
...
def categorical_kl(logp0, logp1):
    """Returns average kl divergence between two batches of dists"""
    all_kls = tf.reduce_sum(input_tensor=tf.exp(logp1) * (logp1 - logp0), axis=1)
    return tf.reduce_mean(input_tensor=all_kls)


def categorical_entropy(logp):
    """Returns average entropy over a batch of dists"""
    all_ents = -tf.reduce_sum(input_tensor=logp * tf.exp(logp), axis=1)
    return tf.reduce_mean(input_tensor=all_ents)
...
def mlp_categorical_policy(x, a, hidden_sizes, activation, output_activation, action_space):
    ...
    pi = tf.squeeze(tf.compat.v1.multinomial(logits=logits, num_samples=1), axis=1)
    logp = tf.reduce_sum(input_tensor=tf.one_hot(a, depth=act_dim) * logp_all, axis=1)
    logp_pi = tf.reduce_sum(input_tensor=tf.one_hot(pi, depth=act_dim) * logp_all, axis=1)
...
def mlp_squashed_gaussian_policy(x, a, hidden_sizes, activation, output_activation, action_space):
    ...
    std = tf.exp(log_std)
    u = mu + tf.random.normal(tf.shape(input=mu)) * std
    pi = tf.tanh(u)
    ...
    def apply_squashing_func(log_prob, raw_action):
        ...
        log_prob -= tf.reduce_sum(input_tensor=2 * (np.log(2) - act - tf.nn.softplus(-2 * act)), axis=1)
        return log_prob
    ...
    # Approximate entropy
    ent = -tf.reduce_mean(input_tensor=logp_pi)  # approximate! hacky!
